...
class LicenseInfo(BaseModel):
    """Информация о лицензии 1С"""

    model_config = ConfigDict(from_attributes=True, populate_by_name=True)

    license_type: str = Field(default="", alias="license-type")
    total: int = 0
    used: int = Field(default=0, alias="in-use")
    free: int = 0
    series: str = ""
    description: str = ""
    ...
    def usage_percent(self) -> float:
        """Процент использования лицензии"""
        if self.total > 0:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "LicenseInfo":
        ...
        return cls(
            **{
                "license-type": data.get("license-type", data.get("license_type", "")),
                "total": int(data.get("total", 0)),
                "in-use": int(data.get("in-use", data.get("used", 0))),
...
class LicenseStats(BaseModel):
    """Статистика по лицензиям 1С"""

    model_config = ConfigDict(from_attributes=True, populate_by_name=True)

    license_type: str  # "local", "server", "hasp", "unknown"
    host: str = "localhost"
    ...
    @property
    def usage_percent(self) -> float:
        """Процент использования всех лицензий"""
        if self.total_licenses > 0:
            return round((self.used_licenses / self.total_licenses) * 100, 2)
        return 0.0

    def to_lld(self) -> List[Dict[str, Any]]:
        """Формат для Zabbix LLD по типам лицензий"""
        lld = []
        for lic in self.licenses:
            lld.append({
                "{#LICENSE.TYPE}": lic.license_type,
                "{#LICENSE.TOTAL}": lic.total,
                "{#LICENSE.USED}": lic.used,
                "{#LICENSE.FREE}": lic.free,
                "{#LICENSE.SERIES}": lic.series,
            })
        return lld

    def to_zabbix(self, cluster_id: str) -> List[Dict[str, Any]]:
        """Формат для Zabbix trapper"""
        return [
            {"key": f"zbx1cpy.license.type[{cluster_id}]", "value": self.license_type},
            {"key": f"zbx1cpy.license.total[{cluster_id}]", "value": self.total_licenses},
            {"key": f"zbx1cpy.license.used[{cluster_id}]", "value": self.used_licenses},
            {"key": f"zbx1cpy.license.free[{cluster_id}]", "value": self.free_licenses},
            {"key": f"zbx1cpy.license.usage_percent[{cluster_id}]", "value": self.usage_percent},
        ]
