...
def get_all_infobases_for_cluster(cluster_id: str) -> List[Dict[str, Any]]:
    ...
    cmd = [str(rac_path), "infobase", "summary", "list", "--cluster", cluster_id, ras_address]
...
def get_detailed_infobase_status(cluster_id: str, infobase_name: str) -> Dict[str, Any]:
    ...
    if not target_infobase:
        return {
            "error": f"Информационная база '{infobase_name}' не найдена в кластере {cluster_id}"
        }
...
def export_monitoring_data_to_json(
    monitoring_data: Dict[str, Any], filename: Optional[str] = None
) -> str:
    """
