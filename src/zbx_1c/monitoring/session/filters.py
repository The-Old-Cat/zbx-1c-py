...
def is_session_active(
    ...
    🎯 Рекомендации по threshold_minutes:
    ─────────────────────────────────
    | Тип сессии | Рекомендуемый порог | Обоснование |
    |------------|---------------------|-------------|
    | Designer (Конфигуратор) | 10-15 мин | Разработчик читает код без вызовов |
    | 1CV8C (Тонкий клиент) | 5 мин | Стандартная сессия пользователя |
    | BackgroundJob | 1-2 мин | Фоновое задание работает интенсивно |
    | SystemBackgroundJob | 1-2 мин | Системное задание |
    | JobScheduler | Всегда активен | Планировщик |
    | WebClient | 5 мин | Веб-клиент |
    | ExternalConn | 3-5 мин | Внешние подключения |

    📊 Примеры использования:
    ─────────────────────────────────
    # 1. Конфигуратор (Designer) — 15 минут
    is_session_active(session, threshold_minutes=15)

    # 2. Фоновое задание — 2 минуты
    is_session_active(session, threshold_minutes=2)

    # 3. Обычная сессия — 5 минут
    is_session_active(session, threshold_minutes=5)
    ...
    Параметры:
        ...
        min_bytes (int): Минимальный объём трафика в байтах

    Возвращает:
        bool: True — сессия активна, False — сессия неактивна
    """
    # -------------------------------------------------------------------------
