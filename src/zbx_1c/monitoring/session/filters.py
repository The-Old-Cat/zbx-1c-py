"""
Модуль для фильтрации активных сессий 1С:Предприятия.

Содержит логику определения «активной» сессии на основе настраиваемых критериев:
1. Статуса спящего режима (поле 'hibernate') — ОБЯЗАТЕЛЬНЫЙ критерий
2. Времени последней активности (поле 'last-active-at') — ОБЯЗАТЕЛЬНЫЙ критерий
3. Фактической активности за последние 5 минут (поле 'calls-last-5min') — ОПЦИОНАЛЬНЫЙ
4. Трафика за последние 5 минут (поле 'bytes-last-5min') — ОПЦИОНАЛЬНЫЙ

⚠️ ВАЖНО: Активная сессия ≠ просто «открытая» сессия.
Активная сессия — это сессия, с которой пользователь реально взаимодействует
в данный момент (последние 5 минут).

📊 Уровни определения активности:
─────────────────────────────────
Уровень 1 (Базовый): Только hibernate
    → is_session_active(session)
    → Считает активными все сессии, не в спящем режиме

Уровень 2 (По времени): hibernate + last-active-at
    → is_session_active(session, threshold_minutes=5)
    → Требует свежую последнюю активность

Уровень 3 (По вызовам): hibernate + last-active-at + calls
    → is_session_active(session, check_activity=True, min_calls=1)
    → Требует фактические вызовы сервера

Уровень 4 (Строгий): Все критерии + трафик
    → is_session_active(session, check_activity=True, check_traffic=True,
                        min_calls=5, min_bytes=1000)
    → Максимально строгая проверка активности

📋 Критерии активности:
─────────────────────────────────
✅ Критерий 1 (ОБЯЗАТЕЛЬНЫЙ): hibernate == 'no'
    Сессия не должна быть в спящем режиме.
    Если hibernate == 'yes' → сессия НЕ активна (сервер её «усыпил»).

✅ Критерий 2 (ОБЯЗАТЕЛЬНЫЙ): last-active-at < threshold_minutes
    Последняя активность должна быть свежее порога (по умолчанию 5 минут).
    Если последняя активность была дольше → сессия НЕ активна.

⚙️ Критерий 3 (ОПЦИОНАЛЬНЫЙ): calls-last-5min >= min_calls
    Проверяется только если check_activity=True.
    Если вызовов меньше min_calls → сессия НЕ активна.

⚙️ Критерий 4 (ОПЦИОНАЛЬНЫЙ): bytes-last-5min >= min_bytes
    Проверяется только если check_traffic=True.
    Если трафика меньше min_bytes → сессия НЕ активна.

🔧 Примеры использования:
─────────────────────────────────
# 1. Базовый мониторинг (все сессии не в hibernate)
active = filter_active_sessions(sessions)

# 2. Строгий мониторинг (только с вызовами >= 10)
active = filter_active_sessions(sessions, check_activity=True, min_calls=10)

# 3. Максимально строгий (вызовы + трафик)
active = filter_active_sessions(
    sessions,
    check_activity=True, min_calls=5,
    check_traffic=True, min_bytes=1000
)

# 4. Для Zabbix (считаем все не-hibernate сессии)
active_sessions = sum(1 for s in sessions if s.get("hibernate") == "no")
"""

from datetime import datetime, timedelta
from typing import List, Dict, Any


# ============================================================================
# ОСНОВНЫЕ ФУНКЦИИ
# ============================================================================


def is_session_active(
    session: Dict[str, Any],
    threshold_minutes: int = 5,
    check_activity: bool = False,
    check_traffic: bool = False,
    min_calls: int = 0,
    min_bytes: int = 0,
) -> bool:
    """
    Определяет, является ли сессия 1С активной, на основе настраиваемых критериев.

    📋 Критерии активности:
    ─────────────────────────────────
    ✅ Критерий 1 (ОБЯЗАТЕЛЬНЫЙ): hibernate == 'no'
        Сессия не должна быть в спящем режиме.
        Если hibernate == 'yes' → сессия НЕ активна (сервер её «усыпил»).

    ✅ Критерий 2 (ОБЯЗАТЕЛЬНЫЙ): last-active-at < threshold_minutes
        Последняя активность должна быть свежее порога (по умолчанию 5 минут).
        Если последняя активность была дольше → сессия НЕ активна.

    ⚙️ Критерий 3 (ОПЦИОНАЛЬНЫЙ): calls-last-5min >= min_calls
        Проверяется только если check_activity=True.
        Если вызовов меньше min_calls → сессия НЕ активна.

    ⚙️ Критерий 4 (ОПЦИОНАЛЬНЫЙ): bytes-last-5min >= min_bytes
        Проверяется только если check_traffic=True.
        Если трафика меньше min_bytes → сессия НЕ активна.

    🎯 Уровни проверки:
    ─────────────────────────────────
    Уровень 1 (Базовый): Только обязательные критерии
        → is_session_active(session)
        → Для мониторинга всех сессий (Zabbix)

    Уровень 2 (С вызовами): + проверка активности
        → is_session_active(session, check_activity=True, min_calls=1)
        → Для определения реально работающих пользователей

    Уровень 3 (Строгий): + проверка трафика
        → is_session_active(session, check_activity=True, check_traffic=True,
                            min_calls=5, min_bytes=1000)
        → Для строгой фильтрации активных сессий

    Параметры:
        session (Dict[str, Any]): Словарь с данными одной сессии
                                  из вывода `rac session list`.
                                  Обязательные поля: 'hibernate', 'last-active-at'.
                                  Рекомендуемые поля: 'calls-last-5min', 'bytes-last-5min'.
        threshold_minutes (int): Порог активности в минутах.
                                 По умолчанию 5 минут.
                                 Сессия считается неактивной, если last-active-at старше этого порога.
        check_activity (bool): Проверять активность по calls-last-5min.
                               False (по умолчанию) — критерий отключен.
                               True — сессия активна только если calls >= min_calls.
        check_traffic (bool): Проверять трафик по bytes-last-5min.
                              False (по умолчанию) — критерий отключен.
                              True — сессия активна только если bytes >= min_bytes.
        min_calls (int): Минимальное количество вызовов за 5 минут для активности.
                         По умолчанию 0 (любое количество).
                         Работает только если check_activity=True.
        min_bytes (int): Минимальный объём трафика в байтах за 5 минут для активности.
                         По умолчанию 0 (любой трафик).
                         Работает только если check_traffic=True.

    Возвращает:
        bool: True — сессия активна (все включённые критерии пройдены),
              False — сессия неактивна или некорректна.

    Примеры использования:
        >>> session = {
        ...     'hibernate': 'no',
        ...     'last-active-at': '2026-02-11T10:06:04',
        ...     'calls-last-5min': '36',
        ...     'bytes-last-5min': '103675'
        ... }

        >>> # Базовая проверка (только hibernate + last-active-at)
        >>> is_session_active(session)
        True

        >>> # Проверка с вызовами (требуется calls >= 10)
        >>> is_session_active(session, check_activity=True, min_calls=10)
        True

        >>> # Строгая проверка (вызовы + трафик)
        >>> is_session_active(session, check_activity=True, check_traffic=True,
        ...                   min_calls=10, min_bytes=1000)
        True

        >>> # Сессия в спящем режиме (не активна)
        >>> session = {
        ...     'hibernate': 'yes',
        ...     'last-active-at': '2026-02-10T15:56:51',
        ...     'calls-last-5min': '0'
        ... }
        >>> is_session_active(session)
        False

    Примечания:
        • Если поля 'calls-last-5min' или 'bytes-last-5min' отсутствуют или некорректны,
          и соответствующий критерий включён (check_activity=True или check_traffic=True),
          функция считает значение равным 0 и возвращает False (если min_calls > 0 или min_bytes > 0).
        • Временные зоны: функция корректно обрабатывает ISO-строки с и без указания временной зоны.
        • Для массового применения используйте filter_active_sessions().
    """
    # -------------------------------------------------------------------------
    # КРИТЕРИЙ 1: Проверка спящего режима
    # -------------------------------------------------------------------------
    # Поле 'hibernate' показывает, переведена ли сессия сервером в энергосберегающий режим.
    # • 'no' — сессия активна (не в спящем режиме)
    # • 'yes' — сессия приостановлена из-за бездействия > 20 минут (по умолчанию)
    #
    # ⚠️ Даже если 'last-active-at' свежий, но 'hibernate' = 'yes' —
    #    сессия НЕ считается активной (сервер её "усыпил").
    if session.get("hibernate") == "yes":
        return False

    # -------------------------------------------------------------------------
    # КРИТЕРИЙ 2: Проверка времени последней активности
    # -------------------------------------------------------------------------
    # Сравниваем 'last-active-at' с текущим временем минус порог (по умолчанию 5 минут).
    try:
        # Преобразуем ISO-строку в объект datetime
        # Примеры входных строк:
        #   '2026-02-11T10:06:04'        → naive datetime (без временной зоны)
        #   '2026-02-11T10:06:04Z'       → UTC (заменяем 'Z' на '+00:00')
        #   '2026-02-11T10:06:04+03:00'  → с указанием временной зоны
        last_active_str = session["last-active-at"].replace("Z", "+00:00")
        last_active = datetime.fromisoformat(last_active_str)

        # Определяем текущее время в той же временной зоне, что и last_active
        # • Если last_active имеет временную зону (tzinfo) — используем её
        # • Иначе — работаем с локальным временем (naive datetime)
        if last_active.tzinfo:
            now = datetime.now(last_active.tzinfo)
        else:
            now = datetime.now()

        # Проверяем, что последняя активность была позже, чем (сейчас - порог)
        if last_active < now - timedelta(minutes=threshold_minutes):
            return False

    except (ValueError, KeyError, TypeError):
        # Обработка ошибок:
        # • ValueError — некорректный формат даты
        # • KeyError — отсутствует поле 'last-active-at'
        # • TypeError — значение не является строкой
        #
        # В случае ошибки считаем сессию НЕАКТИВНОЙ (защита от некорректных данных)
        return False

    # -------------------------------------------------------------------------
    # КРИТЕРИЙ 3: Проверка фактической активности за последние 5 минут (ОПЦИОНАЛЬНО)
    # -------------------------------------------------------------------------
    # Если check_activity=True, проверяем количество вызовов сервера
    if check_activity:
        try:
            calls_str = session.get("calls-last-5min", "0")
            calls = int(calls_str) if calls_str else 0
            if calls < min_calls:
                return False
        except (ValueError, TypeError):
            # Если не удалось преобразовать, считаем что вызовов не было
            if min_calls > 0:
                return False

    # -------------------------------------------------------------------------
    # КРИТЕРИЙ 4: Проверка трафика за последние 5 минут (ОПЦИОНАЛЬНО)
    # -------------------------------------------------------------------------
    # Если check_traffic=True, проверяем объём переданных данных
    if check_traffic:
        try:
            bytes_str = session.get("bytes-last-5min", "0")
            bytes_ = int(bytes_str) if bytes_str else 0
            if bytes_ < min_bytes:
                return False
        except (ValueError, TypeError):
            # Если не удалось преобразовать, считаем что трафика не было
            if min_bytes > 0:
                return False

    # -------------------------------------------------------------------------
    # ИТОГ: Все критерии пройдены → сессия активна
    # -------------------------------------------------------------------------
    return True


def filter_active_sessions(
    sessions: List[Dict[str, Any]],
    threshold_minutes: int = 5,
    check_activity: bool = False,
    check_traffic: bool = False,
    min_calls: int = 0,
    min_bytes: int = 0,
) -> List[Dict[str, Any]]:
    """
    Фильтрует список сессий, оставляя только активные.

    Параметры:
        sessions (List[Dict[str, Any]]): Список словарей с данными сессий.
                                         Каждый словарь должен содержать поля,
                                         ожидаемые функцией is_session_active().
        threshold_minutes (int): Порог активности в минутах. По умолчанию 5.
        check_activity (bool): Проверять активность по calls-last-5min.
        check_traffic (bool): Проверять трафик по bytes-last-5min.
        min_calls (int): Минимальное количество вызовов за 5 минут.
        min_bytes (int): Минимальный объём трафика в байтах за 5 минут.

    Возвращает:
        List[Dict[str, Any]]: Список активных сессий (фильтрованная копия входного списка).

    Пример использования:
        >>> all_sessions = parse_rac_output(rac_stdout)
        >>> # Строгая фильтрация: только сессии с вызовами >= 10 и трафиком >= 1000 байт
        >>> active = filter_active_sessions(all_sessions, check_activity=True, check_traffic=True, min_calls=10, min_bytes=1000)
        >>> print(f"Активных сессий: {len(active)}")
        Активных сессий: 3

    Примечания:
        • Возвращает НОВЫЙ список — исходный список не модифицируется.
        • Пустой входной список → пустой результат (без ошибок).
        • Для каждой сессии вызывается is_session_active().
    """
    return [
        s
        for s in sessions
        if is_session_active(
            s,
            threshold_minutes=threshold_minutes,
            check_activity=check_activity,
            check_traffic=check_traffic,
            min_calls=min_calls,
            min_bytes=min_bytes,
        )
    ]


# ============================================================================
# ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ
# ============================================================================


def get_session_summary(session: Dict[str, Any]) -> str:
    """
    Формирует краткое текстовое описание сессии для логирования или вывода.

    Параметры:
        session (Dict[str, Any]): Словарь с данными сессии.

    Возвращает:
        str: Строка вида "User: Иванов И.И. | App: 1CV8C | Last: 10:06:04 | Calls: 36"

    Пример:
        >>> session = {
        ...     'user-name': 'Иванов Иван Иванович',
        ...     'app-id': '1CV8C',
        ...     'last-active-at': '2026-02-11T10:06:04',
        ...     'calls-last-5min': '36'
        ... }
        >>> get_session_summary(session)
        'User: Иванов И.И.     | App: 1CV8C   | Last: 10:06:04 | Calls: 36'
    """
    user = session.get("user-name", "N/A")
    # Сокращаем ФИО для компактности: "Иванов Иван Иванович" → "Иванов И.И."
    if " " in user:
        parts = user.split()
        if len(parts) >= 3:
            user = f"{parts[0]} {parts[1][0]}.{parts[2][0]}."

    app = session.get("app-id", "N/A")
    last_active = (
        session.get("last-active-at", "N/A").split("T")[-1][:8]
        if "T" in session.get("last-active-at", "")
        else session.get("last-active-at", "N/A")
    )
    calls = session.get("calls-last-5min", "N/A")

    return f"User: {user:20} | App: {app:8} | Last: {last_active} | Calls: {calls}"


# ============================================================================
# ТЕСТОВЫЙ БЛОК (для самостоятельной проверки модуля)
# ============================================================================

if __name__ == "__main__":
    # Пример данных для тестирования (имитация вывода rac session list)
    test_sessions = [  # ← ИМЯ БЕЗ 'session' В КОРНЕ (избегаем глобальной переменной 'session')
        {
            "hibernate": "no",
            "last-active-at": "2026-02-11T10:06:04",
            "calls-last-5min": "36",
            "bytes-last-5min": "103675",
            "user-name": "Иванов Иван Иванович",
            "app-id": "1CV8C",
        },
        {
            "hibernate": "yes",  # ← спящий режим
            "last-active-at": "2026-02-10T15:56:51",
            "calls-last-5min": "0",
            "user-name": "Пользователь2",
            "app-id": "1CV8",
        },
        {
            "hibernate": "no",
            "last-active-at": "2026-02-11T10:06:54",
            "calls-last-5min": "0",  # ← нет вызовов
            "bytes-last-5min": "0",  # ← нет трафика
            "user-name": "Пользователь3",
            "app-id": "Designer",
        },
    ]

    print("Тест фильтрации активных сессий:")
    print("=" * 70)

    active = filter_active_sessions(test_sessions)

    print(f"\nВсего сессий: {len(test_sessions)}")
    print(f"Активных сессий: {len(active)}\n")

    # Используем 's' вместо 'session' в цикле (избегаем переопределения параметра функции)
    for i, s in enumerate(active, 1):
        print(f"{i}. {get_session_summary(s)}")

    print("\n" + "=" * 70)
    print("✅ Тест завершён. Модуль готов к использованию.")
