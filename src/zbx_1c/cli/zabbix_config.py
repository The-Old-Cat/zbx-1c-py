...
# Добавляем пути к src и корню проекта для импорта модулей проекта
sys.path.insert(0, str(Path(__file__).parent.parent))  # Добавляем src как корень
sys.path.insert(0, str(Path(__file__).parent.parent.parent))  # Добавляем корень проекта

from zbx_1c.core.config import settings


def check_executable_access(path: Union[str, Path]) -> Tuple[bool, str]:
...
def check_log_directory(log_path: Union[str, Path]) -> Tuple[bool, str]:
